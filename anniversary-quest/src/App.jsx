...
export default function App() {
  ...
  // Canvas Game Render Loop (Includes Boss 3 Storm & Rain)
  useEffect(() => {
    ...
    const render = () => {
      ...
      const skyGradients = [
        ['#1e1b4b', '#312e81'],
        ['#064e3b', '#065f46'],
        ['#180521', '#3b0738'], // Dark ominous storm sky for Boss 3
      ];
      ...
      if (bossFlash) {
        ...
        if (phase === 1) {
          ...
          // Boss 3: Glowing Overthink Phantom
  ...
  return (
    <div className="w-full min-h-screen h-[100dvh] flex items-center justify-center font-cozy text-white select-none bg-black overflow-hidden p-0 sm:p-2">
      {/* Universal Responsive Container */}
      <div className={`w-full max-w-[430px] h-[100dvh] max-h-[920px] flex flex-col justify-between relative bg-slate-950 border-x border-slate-800 shadow-2xl overflow-hidden ${screenShake ? 'animate-shake' : ''}`}>

        {/* Global Sound & Rules Header Bar */}
        <div className="absolute top-2 right-3 z-40 flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-full bg-black/60 border border-slate-700 text-pink-300 hover:text-white backdrop-blur-md transition-all active:scale-90"
            title={isMuted ? "Unmute Music" : "Mute Music"}
          >
            {isMuted ? <VolumeX size={15} /> : <Volume2 size={15} className="text-yellow-300 animate-pulse" />}
          </button>
        </div>

        {/* ================= SCREEN 1: LANDING & BRIEFING ================= */}
        {gameState === 'landing' && (
          <div className="w-full h-full flex flex-col items-center justify-between p-4 sm:p-6 text-center overflow-y-auto">
            <div className="w-full flex justify-between items-center pt-1">
              <span className="font-pixel text-[8px] sm:text-[9px] text-pink-400 bg-pink-950/70 border border-pink-700/60 px-3 py-1 rounded-full">
                MAY 9, 2026 ➔ TODAY ❤️
              </span>
              <button 
                onClick={() => { initAudio(); playSound(500, 'sine', 0.05); setShowInstructions(true); }}
                className="flex items-center gap-1 text-xs text-pink-300 hover:text-white bg-white/10 px-3 py-1 rounded-full backdrop-blur-md mr-8"
              >
                <HelpCircle size={13} /> Rules
              </button>
            </div>

            <div className="flex flex-col items-center my-auto gap-2.5 sm:gap-3.5 w-full">
              ...
              <div>
                <h1 className="font-pixel text-sm sm:text-base text-pink-200 tracking-wider leading-relaxed">
                  HAPPY 4 MONTHS!<br />OUR ANNIVERSARY QUEST
                </h1>
                <p className="text-[11px] sm:text-xs text-pink-300/80 mt-0.5">
                  Dating Since May 9, 2026 • 123 Days of Love
                </p>
              </div>

              {/* Responsive 2-Row Stacked Quest Intel Card */}
              <div className="bg-purple-950/80 border-2 border-purple-800 rounded-2xl p-3 sm:p-4 w-full max-w-[340px] text-left shadow-2xl backdrop-blur-md space-y-2">
                <div className="font-pixel text-[7.5px] sm:text-[8px] text-yellow-300 border-b border-purple-800/80 pb-1.5 flex items-center justify-between tracking-wider">
                  <span>QUEST COMBOS</span>
                  <span className="text-pink-300">INPUT SEQUENCE</span>
                </div>

                {/* Level 1 */}
                <div className="bg-purple-900/40 p-2 sm:p-2.5 rounded-xl border border-purple-700/50 space-y-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="font-pixel text-[7.5px] sm:text-[8px] text-pink-300 bg-pink-950 border border-pink-700/70 px-2 py-0.5 rounded-md flex items-center gap-1">
                      ⚡ TAP
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-purple-900/40 p-2 sm:p-2.5 rounded-xl border border-purple-700/50 space-y-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="font-pixel text-[7.5px] sm:text-[8px] text-pink-300 bg-pink-950 border border-pink-700/70 px-2 py-0.5 rounded-md">
                      ⚡ TAP
                    </span>
                    <span className="text-xs text-purple-400 font-bold">+</span>
                    <span className="font-pixel text-[7.5px] sm:text-[8px] text-emerald-300 bg-emerald-950 border border-emerald-700/70 px-2 py-0.5 rounded-md">
                      💓 MASH
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-purple-900/40 p-2 sm:p-2.5 rounded-xl border border-purple-700/50 space-y-1">
                  ...
                  <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap">
                    <span className="font-pixel text-[7px] sm:text-[7.5px] text-pink-300 bg-pink-950 border border-pink-700/70 px-1.5 py-0.5 rounded-md">
                      ⚡ TAP
                    </span>
                    <span className="text-xs text-purple-400 font-bold">+</span>
                    <span className="font-pixel text-[7px] sm:text-[7.5px] text-emerald-300 bg-emerald-950 border border-emerald-700/70 px-1.5 py-0.5 rounded-md">
                      💓 MASH
                    </span>
                    <span className="text-xs text-purple-400 font-bold">+</span>
                    <span className="font-pixel text-[7px] sm:text-[7.5px] text-purple-300 bg-purple-950 border border-purple-700/70 px-1.5 py-0.5 rounded-md">
                      🪄 SWIPE
                    </span>
                  </div>
                </div>
            ...
            <button
              ...
              className="w-full font-pixel text-[11px] sm:text-xs bg-gradient-to-r from-pink-500 via-rose-500 to-pink-600 hover:from-pink-600 hover:to-rose-600 py-3.5 sm:py-4 rounded-xl shadow-lg shadow-pink-500/30 flex items-center justify-center gap-2 tracking-wider transform active:scale-95 transition-all mt-2"
            >
              START BATTLE <ArrowRight size={15} />
            </button>
          </div>
        )}
        ...
        {/* ================= INSTRUCTIONS MODAL ================= */}
        {showInstructions && (
          <div className="absolute inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6">
            <div className="bg-slate-900 border-2 border-pink-500/80 rounded-2xl p-5 max-w-[320px] w-full text-left relative shadow-2xl">
              ...
              <div className="text-xs text-slate-300 space-y-2.5 leading-relaxed">
                ...
                <p>💓 <b>LEVEL 2:</b> Tap circle + rapidly mash the button to fill the heart meter!</p>
                <p>🪄 <b>LEVEL 3:</b> Circle tap + Mash heart + rapidly swipe screen during the pouring rainstorm!</p>
        ...
        {gameState === 'battle' && (
          <div className="w-full h-full flex flex-col justify-between relative">
            ...
            <div className="relative flex-1 w-full bg-slate-900 overflow-hidden">
              ...
              {/* Stage Badge */}
              <div className="absolute top-3 left-3 bg-black/80 border border-slate-700 px-2 py-0.5 rounded-md font-pixel text-[7.5px] text-yellow-300">
                STAGE {phase}/3 {phase === 3 ? '🌧️' : ''}
              </div>

              {/* Boss HP Bar */}
              <div className="absolute top-3 right-12 bg-black/85 border border-slate-700 p-1.5 sm:p-2 rounded-lg w-36 sm:w-44 shadow-lg">
                <div className="flex justify-between font-pixel text-[7.5px] sm:text-[8px] text-pink-300 mb-1">
                  <span className="truncate max-w-[85px]">{currentBoss.name}</span>
                  <span>{bossHp}%</span>
                </div>
              ...
              {/* Team Health Bar */}
              <div className="absolute bottom-3 left-3 bg-black/85 border border-slate-700 p-1.5 sm:p-2 rounded-lg w-40 sm:w-44 shadow-lg">
                <div className="flex justify-between font-pixel text-[7.5px] sm:text-[8px] text-emerald-400 mb-1">
              ...
              {finalBossIntro && (
                <div className="absolute inset-0 z-50 bg-red-950 flex flex-col items-center justify-center animate-pulse text-center p-6">
                  ...
                  <span className="font-pixel text-[8px] sm:text-[9px] text-yellow-400 bg-black/80 px-3 py-1 rounded-full border border-yellow-500 mb-3 animate-bounce">
                    🌧️ FINAL TRIAL: RAINSTORM 🌧️
                  </span>
                  <h2 className="font-pixel text-sm sm:text-base text-white tracking-widest leading-relaxed drop-shadow-[0_4px_10px_rgba(239,68,68,0.8)]">
              ...
              {qteStep === 'mash' && (
                <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/55 backdrop-blur-[1px] p-4">
                  <div className="bg-slate-900 border-2 border-pink-500 rounded-2xl p-4 flex flex-col items-center max-w-[260px] w-full text-center shadow-2xl">
                    ...
                    <p className="text-xs text-pink-200 mb-2 font-semibold">Tap repeatedly to fill the heart!</p>

                    <div className="relative w-16 h-16 flex items-center justify-center mb-2">
                      <div 
                        className="w-14 h-14 rounded-full bg-pink-500/20 border-2 border-pink-400 flex items-center justify-center overflow-hidden transition-all"
                      ...
                      >
                        ...
                        />
                        <span className="relative z-10 text-xl animate-pulse">💖</span>
                      </div>
                    </div>
              ...
              {qteStep === 'swipe' && (
                ...
                >
                  <div className="bg-slate-900 border-2 border-purple-500 rounded-2xl p-4 flex flex-col items-center max-w-[270px] w-full text-center shadow-2xl pointer-events-none">
                    ...
                    <p className="text-xs text-purple-200 mb-2 font-semibold">Swipe left & right rapidly in rain!</p>

                    <div className="text-3xl my-1 animate-pulse">🪄🌧️</div>

                    <div className="flex gap-2 my-2">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <div 
                          key={s} 
                          className={`w-5 h-5 rounded-full border border-purple-400 flex items-center justify-center text-[9px] ${
                    ...
                    <span className="font-pixel text-[7px] text-purple-300/80 mt-2">SWIPE SCREEN NOW! ➔ ⬅️</span>
                  </div>
                </div>
              )}
              ...
              {showNeedleMinigame && (
                <div className="absolute inset-0 z-40 bg-black/85 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-fade-in">
                  <div className="bg-slate-900 border-2 border-red-500/90 rounded-2xl p-4 w-full max-w-[280px] flex flex-col items-center text-center shadow-2xl">
                    <span className="font-pixel text-[8px] text-red-400 flex items-center gap-1 bg-red-950/80 px-2.5 py-1 rounded-full border border-red-500/50 mb-2">
                      <AlertTriangle size={12} /> CRITICAL HEALTH ALERT!
                    </span>
                    <p className="text-xs text-pink-100 font-semibold mb-2">
                      Stop the needle in the green center to heal!
                    </p>

                    <div className="relative w-12 h-40 bg-slate-950 border-2 border-slate-700 rounded-full overflow-hidden my-2 flex items-center justify-center shadow-inner">
                    ...
                    <button
                      ...
                      className="w-full mt-2 font-pixel text-[9px] bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white py-2.5 rounded-xl shadow-lg shadow-red-500/30 active:scale-95 transition-all"
                    >
                      STOP NEEDLE! 🎯
                    </button>
                  </div>
              ...
              {showTriviaModal && (
                <div className="absolute inset-0 z-40 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
                  <div className="bg-slate-900 border-2 border-pink-500 rounded-2xl p-5 w-full max-w-[310px] text-center shadow-2xl">
                    <div className="font-pixel text-[8px] text-pink-400 bg-pink-950/80 px-3 py-1 rounded-full inline-flex items-center gap-1 mb-2 border border-pink-600/50">
                      <Sparkles size={11} className="text-yellow-300" /> RAY'S LOVE TRIVIA
                    </div>
                    <h3 className="text-xs sm:text-sm font-bold text-pink-100 mb-1 leading-snug">
            ...
            {attackWarning && (
              <div className="absolute top-14 left-3 right-3 z-30 bg-red-600/95 text-white font-pixel text-[7.5px] leading-tight px-3 py-2 rounded-xl border border-red-300 flex items-center justify-center gap-2 text-center shadow-2xl animate-bounce">
                <AlertTriangle size={13} className="shrink-0 text-yellow-300" />
                <span>{attackWarning}</span>
              </div>
            )}

            {/* Bottom Tactical Deck */}
            <div className="h-[46%] max-h-[310px] bg-gradient-to-b from-slate-900 via-slate-950 to-black border-t-2 border-slate-700 p-2.5 sm:p-3 flex flex-col justify-between">
              
              <div className="bg-black/80 border border-slate-800 rounded-xl p-1.5 sm:p-2 min-h-[38px] flex items-center justify-center text-center">
                <p className="text-[11px] sm:text-xs text-pink-100 font-semibold leading-snug">
                  {battleLog}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 px-1">
                <span className="font-pixel text-[7.5px] sm:text-[8px] text-amber-300 flex items-center gap-1">
                  <Zap size={10} /> SYNERGY:
                </span>
                ...
                <button
                  ...
                  className={`font-pixel text-[7.5px] sm:text-[8px] px-2.5 py-1 rounded-md transition-all ${
                    synergy >= 100 
              ...
              <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                <button
                  ...
                  className="bg-pink-600 hover:bg-pink-500 disabled:opacity-40 py-2 sm:py-2.5 px-2 rounded-xl flex items-center justify-start gap-2 active:scale-95 transition-all shadow-md shadow-pink-600/30 text-left"
                >
                  <span className="text-lg sm:text-xl">💌</span>
                  <div>
                    <div className="font-pixel text-[7.5px] sm:text-[8px]">COMMUNICATE</div>
                    <div className="text-[9.5px] sm:text-[10px] text-pink-200/80">Power of Love</div>
                  </div>
                </button>
                ...
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 py-2 sm:py-2.5 px-2 rounded-xl flex items-center justify-start gap-2 active:scale-95 transition-all shadow-md shadow-emerald-600/30 text-left"
                >
                  <span className="text-lg sm:text-xl">🍔</span>
                  <div>
                    <div className="font-pixel text-[7.5px] sm:text-[8px]">BURGER & TEA</div>
                    <div className="text-[9.5px] sm:text-[10px] text-emerald-200/80">Snack Attack</div>
                  </div>
                </button>
                ...
                <button
                  ...
                  className="bg-purple-600 hover:bg-purple-500 disabled:opacity-40 py-2 sm:py-2.5 px-2 rounded-xl flex items-center justify-start gap-2 active:scale-95 transition-all shadow-md shadow-purple-600/30 text-left"
                >
                  <span className="text-lg sm:text-xl">🫂</span>
                  <div>
                    <div className="font-pixel text-[7.5px] sm:text-[8px]">WARM HUG</div>
                    <div className="text-[9.5px] sm:text-[10px] text-purple-200/80">Reassurance</div>
                  </div>
                </button>
                ...
                <button
                  disabled={isTurnLocked || qteStep || showNeedleMinigame || showTriviaModal || finalBossIntro}
                  onClick={executeHeal}
                  className="bg-sky-600 hover:bg-sky-500 disabled:opacity-40 py-2 sm:py-2.5 px-2 rounded-xl flex items-center justify-start gap-2 active:scale-95 transition-all shadow-md shadow-sky-600/30 text-left"
                >
                  <span className="text-lg sm:text-xl">🧋</span>
                  <div>
                    <div className="font-pixel text-[7.5px] sm:text-[8px]">WARM MILK TEA</div>
                    <div className="text-[9.5px] sm:text-[10px] text-sky-200/80">+35 Team HP</div>
                  </div>
                </button>
              </div>
            </div>
        ...
        {/* ================= SCREEN 3: GAME OVER ================= */}
        {gameState === 'gameover' && (
          <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center bg-black/95 animate-fade-in gap-4 sm:gap-5">
            <Skull size={44} className="text-red-500 animate-bounce" />
            <div>
              <h2 className="font-pixel text-base sm:text-lg text-red-500 tracking-wider">GAME OVER</h2>
            ...
            <div className="w-full max-w-[280px] flex flex-col gap-2.5 sm:gap-3">
              <button
                onClick={() => resetCampaign(false)}
                className="w-full font-pixel text-[11px] sm:text-xs bg-red-600 hover:bg-red-500 py-3.5 px-6 rounded-xl shadow-lg shadow-red-600/40 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="w-full font-pixel text-[9px] sm:text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 px-6 rounded-xl border border-slate-700 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                <HelpCircle size={14} /> MAIN MENU & RULES
              </button>
            </div>
          </div>
        )}

        {/* ================= SCREEN 4: VICTORY & REWARD (SCRATCH TICKET REMOVED) ================= */}
        {gameState === 'victory' && (
          <div className="w-full h-full flex flex-col items-center justify-between p-4 sm:p-5 text-center overflow-y-auto">
            
            <div className="w-full flex flex-col items-center pt-1">
              <span className="font-pixel text-[8px] sm:text-[9px] text-yellow-300 bg-yellow-950/70 border border-yellow-600/60 px-3 py-1 rounded-full flex items-center gap-1.5 shadow-md">
                <Trophy size={12} /> CAMPAIGN CLEARED!
              </span>
              <h2 className="font-pixel text-xs sm:text-sm text-pink-200 mt-2">LEVEL 4 RELATIONSHIP UNLOCKED</h2>
              <p className="text-[11px] sm:text-xs text-pink-300/80 mt-0.5">Happy 4-Month Anniversary, Charisse! ❤️</p>
            </div>

            {/* Interactive Flipped Memory Cards */}
            <div 
              onClick={() => {
                initAudio();
              ...
              className="bg-white p-3 pt-3 pb-3 sm:pb-4 rounded-xl text-slate-800 shadow-2xl max-w-[290px] w-full my-1.5 sm:my-2 transform hover:rotate-1 transition-all cursor-pointer border border-slate-200"
            >
              <div className="w-full h-36 sm:h-40 bg-pink-100 rounded-lg flex flex-col items-center justify-center p-3 sm:p-4 border border-slate-200 text-center">
                <span className="text-3xl sm:text-4xl mb-1.5">{polaroids[activePolaroid].emoji}</span>
                <p className="text-[11px] sm:text-xs text-slate-700 font-semibold italic leading-relaxed">
                  "{polaroids[activePolaroid].note}"
                </p>
              </div>
              <div className="flex justify-between items-center mt-2 px-1">
                <span className="text-[11px] sm:text-xs font-bold text-slate-800 truncate mr-2">{polaroids[activePolaroid].caption}</span>
                <span className="text-[9px] sm:text-[10px] text-slate-400 font-pixel shrink-0">[{activePolaroid + 1}/3] ➔</span>
              </div>
            </div>

            {/* Romantic Anniversary Honor Certificate (Replaces Scratch-off) */}
            <div className="w-full max-w-[310px] bg-gradient-to-br from-purple-950 via-slate-900 to-pink-950 border-2 border-pink-500/80 rounded-2xl p-3.5 text-center relative overflow-hidden shadow-xl">
              <div className="flex items-center justify-between text-yellow-300 font-pixel text-[8px] mb-2 border-b border-pink-500/30 pb-1.5">
                <span className="flex items-center gap-1"><Award size={11} /> 123 DAYS OF LOVE</span>
                <span>SINCE MAY 9, 2026</span>
              </div>

              <div className="flex items-center justify-center gap-1.5 my-1 text-rose-400">
                <Heart size={16} fill="#f43f5e" />
                <span className="font-pixel text-[9px] text-pink-200">OFFICIAL REWARD SCROLL</span>
                <Heart size={16} fill="#f43f5e" />
              </div>

              <p className="text-xs text-slate-200 font-medium leading-relaxed px-2 mt-1">
                Thank you for being my constant warmth, biggest cheerleader, and loving partner through everything. Here's to countless more months and stages cleared together!
              </p>

              <div className="mt-2.5 font-pixel text-[9px] text-emerald-400 bg-emerald-950/70 border border-emerald-500/60 py-1.5 px-2 rounded-lg">
